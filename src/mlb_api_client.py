...
def get_confirmed_lineup(game_pk):
    ...
    NOTE: the battingOrder encoding (first digit = lineup spot, last two
    digits = substitution sequence within that spot, "00" = the starter) is
    documented community knowledge about this API, not something I could
    verify live. Spot-check this against a real boxscore response.
    """
...
def get_probable_pitchers(game_pk):
    """Returns {"home": {player_id, name, hand}, "away": {...}} from the boxscore."""
    raw = _get(f"/game/{game_pk}/boxscore")
    result = {}
    for side in ("home", "away"):
